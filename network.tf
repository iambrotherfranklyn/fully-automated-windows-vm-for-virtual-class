...
resource "azurerm_network_security_group" "lab_subnet_nsg" {
  name                = var.nsg_name
  location            = var.location
  resource_group_name = "${local.owners}-${var.resource_group_name}"
}
...
resource "azurerm_network_security_rule" "lab_nsg_rule_inbound" {
  ...
  resource_group_name         = "${local.owners}-${var.resource_group_name}"
  network_security_group_name = azurerm_network_security_group.lab_subnet_nsg.name
}

# Resource-3: Associate NSG and Subnet
resource "azurerm_subnet_network_security_group_association" "lab_subnet_nsg_associate" {
  depends_on                = [azurerm_network_security_rule.lab_nsg_rule_inbound]
  subnet_id                 = azurerm_subnet.lab_subnet.id
  network_security_group_id = azurerm_network_security_group.lab_subnet_nsg.id
}